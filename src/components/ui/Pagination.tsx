        ...
        <div className="flex items-center gap-0.5 p-0.5 bg-[#F6F6F6] rounded-md">
          {getPageNumbers().map((page, index) =>
            typeof page === 'string' ? (
              <div
                key={`ellipsis-${index}`}
                className="w- h-6 flex items-center justify-center text-[#1B1B1B] font-inter text-sm font-medium"
              >
                {page}
              </div>
            ) : (
              <Button
                ...
                disabled={isLoading}
                className={`w-full px-2 h-6 flex items-center justify-center rounded-md text-sm font-inter font-medium transition-colors ${page === currentPage
